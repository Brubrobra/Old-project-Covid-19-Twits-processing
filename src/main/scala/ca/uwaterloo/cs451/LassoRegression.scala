...
spark-submit --class ca.uwaterloo.cs451.COVID19Predictor.LassoRegression \\ 
...
object LassoRegression {
  ...
  def main(argv: Array[String]) {
    ...
    val train = scaledFeatures
      .filter(col("grams.parsedDate").lt(lit("2020-10-02")))
    ...
    val test = scaledFeatures
      .filter(col("grams.parsedDate").gt(lit("2020-10-01")))
    ...
    val lrModel = LassoWithSGD.train(train, 100, 2.0, 0.3)
    lrModel.save(sc, args.output() + "/lasso_model.model")
